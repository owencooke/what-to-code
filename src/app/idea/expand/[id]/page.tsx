...
import { IdeaCard, IdeaSkeletonCard } from "@/components/cards/IdeaCard";
import ky from "ky";

export default function ExpandIdea({ params }: { params: { id: number } }) {
  const router = useRouter();
  const { idea, setIdea, resetState, features, frameworks } =
    useCreateProjectStore();

  useEffect(() => {
    async function fetchIdea() {
      if (params.id && params.id !== idea?.id) {
        const newIdea = await ky
          .get(`/api/idea/${params.id}`)
          .json<PartialIdea>();

        // Reset the state only if the idea changes
        if (idea && newIdea.id !== idea.id) {
          resetState();
        }
        setIdea(newIdea);
      }
    ...
    fetchIdea();
  }, [params.id, idea?.id, resetState, setIdea]);
    ...
    <div className="flex flex-col justify-center items-center min-h-screen w-full py-8 px-2">
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-12">
          ...
          >
            {idea ? <IdeaCard idea={idea} /> : <IdeaSkeletonCard />}
          </motion.div>
        </div>
        <motion.div
